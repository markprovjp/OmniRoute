...
| `MAX_BODY_SIZE_BYTES`                   | `209715200` (200 MB)    | `src/shared/middleware/bodySizeGuard.ts` | Maximum allowed request body size. Rejects payloads exceeding this limit. Keep the reverse proxy limit at least this large.                                                                                                                                                                                                                                        |
| `OMNIROUTE_PROXY_CLIENT_MAX_BODY_SIZE`  | `200mb`                 | `next.config.mjs`                        | Next.js request-clone limit. Keep this at least as large as `MAX_BODY_SIZE_BYTES`; changing it requires an application restart.                                                                                                                                                                                                                                    |
| `OMNIROUTE_SERVER_ACTIONS_BODY_LIMIT`   | `200mb`                 | `next.config.mjs`                        | Next.js Server Actions body limit for multipart and form requests; changing it requires an application restart.                                                                                                                                                                                                                                                    |
...
MAX_BODY_SIZE_BYTES=209715200  # 200 MB limit for large multi-file requests
```
...
| `OMNIROUTE_CGPT_WEB_IMAGE_TIMEOUT_MS`   | `360000` (6 min)                                                | `open-sse/executors/chatgpt-web.ts`         | Max wait time for an async chatgpt-web image to land via the celsius WebSocket. Increase during upstream queue-deep windows.                                                                                                                                                                      |
...
| `CODEX_SYNTHETIC_SESSION_MAX_CONCURRENCY` | `4`                                           | Maximum parallel non-streaming Codex requests for one API key and synthetic input-prefix session; excess requests fail fast instead of creating an upstream retry staircase. |
| `CODEX_API_KEY_MAX_CONCURRENCY`           | `4`                                           | Maximum active Codex requests for one authenticated API key across session types; protects memory while keeping separate API keys isolated.                                  |
| `CODEX_PROCESS_MAX_CONCURRENCY`           | `6`                                           | Process-wide Codex safety fuse across API keys; used only after per-key isolation is insufficient to protect the Node.js heap.                                               |
...
| `DB_BACKUP_MAX_FILES`                       | `12`                                  | `src/lib/db/backup.ts`                              | Maximum SQLite backup files retained on disk.                                              |
| `DB_BACKUP_RETENTION_DAYS`                  | `0`                                   | `src/lib/db/backup.ts`                              | Maximum age (days) of retained backups. `0` disables age-based pruning.                    |
| `OMNIROUTE_RUNTIME_MAINTENANCE_INTERVAL_MS` | `3600000`                             | `src/lib/jobs/runtimeMaintenanceJob.ts`             | Interval for periodic cleanup of app logs, log tables, call-log artifacts, and DB backups. |
| `OMNIROUTE_DISABLE_RUNTIME_MAINTENANCE`     | `false`                               | `src/lib/jobs/runtimeMaintenanceJob.ts`             | Disable only the periodic runtime maintenance cleanup job.                                 |
