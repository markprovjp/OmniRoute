...
test("getProviderCredentials temporarily spills a busy Codex affinity to an idle account", async () => {
  await settingsDb.updateSettings({ fallbackStrategy: "round-robin", stickyRoundRobinLimit: 10 });
  const first = await seedConnection("codex", {
  ...
  assert.equal(spillover.connectionId, second.id);

  busyLease.release();
  const returned = await auth.getProviderCredentials("codex", null, null, "gpt-5.6-sol", {
    sessionKey,
  });
  assert.equal(returned.connectionId, first.id);
});

test("getProviderCredentials never spills outside allowedConnections", async () => {
  const first = await seedConnection("codex", {
