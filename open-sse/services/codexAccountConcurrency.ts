...
export function getCodexAccountMaxConcurrent(configured?: number | null): number {
  if (configured != null) return clampMaxConcurrent(configured);
...
export function acquireCodexAccountConcurrency(
  ...
  if (!lease) throw new CodexAccountConcurrencyError(connectionId, limit);
  return lease;
}
  ...
  const wrappedBody = new ReadableStream<Uint8Array>({
    ...
    async cancel(reason) {
      ...
      try {
        ...
        // Cancellation is best-effort after releasing the lease.
      }
    },
  });
