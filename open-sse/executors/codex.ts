...
function normalizeCodexTools(
  ...
  body.tools = body.tools.filter((toolValue) => {
    ...
    // Native Codex clients expose local edit/shell/sub-agent tools as Responses
    // custom tools, for example `{ type: "custom", name: "apply_patch" }`.
    // Keep them only in native passthrough; translated OpenAI-compatible traffic
    // still gets sanitized below.
    if (options.preserveNativeCustomTools && (toolType === "custom" || toolType === "command")) {
      const name = typeof tool.name === "string" ? tool.name.trim() : "";
      if (!name) return false;
      tool.name = name.slice(0, 128);
      validToolNames.add(tool.name as string);
      return true;
    }
